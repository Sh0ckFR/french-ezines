...
///////////////////////////////////////////////////////////////////////////////
   ...
   if (!preg_match($s,getenv("REMOTE_ADDR")) && !preg_match($s,gethostbyaddr(getenv("REMOTE_ADDR")))) 
      exit("<h1><a href=http://php.spb.ru/remview/>RemoteView</a>: Access Denied - your host not allow</h1>\n");
   if ($login!==false && (!isset($HTTP_SERVER_VARS['PHP_AUTH_USER']) || 
      ...
      header("WWW-Authenticate: Basic realm=\"RemoteView\"");
      header("HTTP/1.0 401 Unauthorized");
      exit("<h1><a href=http://php.spb.ru/remview/>RemoteView</a>: Access Denied - password erroneous</h1>\n");
   }
...
//   echo " onMouseOver='this.style.value=\\\"line2\\\"' onMouseOut='this.style.value=\\\"line\\\"'>";
   ...
   $button_help=array(
   'up'=>"Dossier prйcйdent",
   'refresh'=>"Actualiser",
   'mode'=>'Setup - Options des dossiers',
   'edit'=>'Infos dossier',
   'home'=>'HomePage',
   'papki'=>'TREE',
   ...
   for ($i=0; $i<count($buttons); $i++) {
      if ($buttons[$i]=='full') {
         ...
         style="font-size: 11pt;">Remote View <br />By Borondin & CrackDa</font></td>';
         continue;
      }
      if ($buttons[$i]=='disk') {
         if (!$win) continue;
   ...
   <font size=1 style='Font: 8pt Verdana'>phpRemoteView &copy; Dmitry Borodin & CrackDa (".mm("version")." $version)<br>
   ".mm("Free download")." - <a href='mailto:i_am_crackda@msn.com/'>i_am_crackda@msn.com</a></b></font></td>
   </tr></table>";
